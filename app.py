...
def login():
    try:
        ...
        if '@' in username_or_email:
            user = conn.execute('SELECT * FROM users WHERE email=?', (username_or_email,)).fetchone()
        else:
            user = conn.execute('SELECT * FROM users WHERE username=?', (username_or_email,)).fetchone()
...
def register():
    try:
        ...
        conn = get_db_connection()
        conn.execute('INSERT INTO users (username, password, email, phone, address) VALUES (?, ?, ?, ?, ?)',
                     (username, hashed_password.decode('utf-8'), email, phone, address))
        conn.commit()
        conn.close()
...
def reset_password():
    try:
        ...
        conn = get_db_connection()
        user = conn.execute('SELECT * FROM users WHERE email=? AND confirmation_code=?', (email, confirmation_code)).fetchone()

        if user:
            hashed_password = bcrypt.hashpw(new_password.encode('utf-8'), bcrypt.gensalt())
            conn.execute('UPDATE users SET password=?, confirmation_code=NULL WHERE email=?', (hashed_password.decode('utf-8'), email))
            conn.commit()
            conn.close()
